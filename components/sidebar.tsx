...
import { ChefHat, ShoppingBag, BarChart3, User, Plus, MessageSquare } from "lucide-react"
...
export function CookDashboardSidebar({ activeTab, setActiveTab }: CookDashboardSidebarProps) {
  return (
    <aside className="w-64 border-r bg-muted/30 min-h-[calc(100vh-4rem)]">
      <div className="p-6 space-y-6">
        ...
        {/* Navigation */}
        <div className="space-y-3">
          <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider">Dashboard</h3>
            ...
            {sidebarItems.map((item) => (
              <Button
                ...
                className={`w-full justify-start ${
                  activeTab === item.id ? "bg-primary/10 text-primary hover:bg-primary/20" : "hover:bg-muted"
                }`}
              ...
              >
                ...
                {item.badge && (
                  <Badge variant="secondary" className="bg-accent/10 text-accent border-accent/20 ml-2">
                    {item.badge}
                  </Badge>
                )}
              </Button>
