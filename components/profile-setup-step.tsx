...
export function ProfileSetupStep({
	...
	const handleKitchenImagesChange = (
		e: React.ChangeEvent<HTMLInputElement>
	) => {
		const files = Array.from(e.target.files || []);
		if (files.length > 0) {
			const currentImages = formData.kitchenImages || [];
			updateFormData({ kitchenImages: [...currentImages, ...files] });
		...
		<div className="space-y-6">
			<div className="space-y-2">
				...
				<p className="text-muted-foreground">
					Create an attractive profile that showcases your cooking skills and
					kitchen to potential customers.
				</p>
			</div>
				...
				<div className="flex items-start space-x-6">
					...
					<div className="space-y-2">
						...
						<p className="text-xs text-muted-foreground">
							Upload a clear, friendly photo of yourself. This helps build trust
							with customers.
						</p>
					</div>
				</div>
			...
			<div className="space-y-4">
				...
				<p className="text-sm text-muted-foreground">
					Upload 2-5 photos of your kitchen to show customers your clean,
				...
				<p className="text-xs text-muted-foreground">
					Recommended: Include photos of your cooking area, storage, and any
