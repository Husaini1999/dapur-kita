...
import { Minus, Plus, Trash2, ShoppingBag, ArrowLeft, Truck, Clock } from "lucide-react"

export function CartPage() {
  const { state, updateQuantity, removeItem, updateInstructions, getItemsByCook } = useCart()
      ...
      <div className="container max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="font-serif font-bold text-2xl sm:text-3xl text-foreground">Your cart is empty</h1>
  ...
  return (
    <div className="container max-w-6xl mx-auto py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
      <div className="flex items-center space-x-4 mb-6 sm:mb-8">
        ...
        <h1 className="font-serif font-bold text-2xl sm:text-3xl text-foreground">Your Cart</h1>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
          {Object.entries(itemsByCook).map(([cookId, items]) => {
            ...
            const cookTotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0)
                      ...
                      <div className="flex-1 space-y-2 min-w-0">
                        <div className="flex items-start justify-between">
                          <div className="min-w-0 flex-1">
                            <h3 className="font-medium text-sm sm:text-base line-clamp-1">{item.dishName}</h3>
                            <p className="text-xs sm:text-sm text-muted-foreground">RM {item.price.toFixed(2)} each</p>
                          </div>
                          <Button
                            variant="ghost"
                            size="sm"
              ...
              <div className="space-y-2">
                ...
                <div className="flex space-x-2">
                  <Input
                    ...
                    className="text-sm"
                  />
                  <Button variant="outline" className="bg-transparent text-sm px-3">
                    Apply
                  </Button>
                </div>
              </div>

              <Button asChild className="w-full bg-primary hover:bg-primary/90 h-11 sm:h-12" size="lg">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-medium text-sm">Free delivery on orders over RM 30</div>
                  <div className="text-xs text-muted-foreground">
                    Add RM {Math.max(0, 30 - subtotal).toFixed(2)} more to qualify
                  </div>
                </div>
              </div>
            </CardContent>
